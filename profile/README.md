<div align="center">

# Awesome-Embedded-Learning-Studio

**面向中文学习者的嵌入式系统开源学习工作室**

### Keep it small, make it fun ✨

</div>

---

<p align="center">
  <!-- 技术栈 -->
  <img alt="C++" src="https://img.shields.io/badge/C++-modern-blueviolet?logo=c%2B%2B&logoColor=white" />
  <img alt="C" src="https://img.shields.io/badge/C-language-blue?logo=c&logoColor=white" />
  <img alt="STM32" src="https://img.shields.io/badge/STM32-MCU-success?logo=arm&logoColor=white" />
  <img alt="Embedded Linux" src="https://img.shields.io/badge/Embedded--Linux-Linux-lightgrey?logo=linux&logoColor=white" />
  <img alt="Qt" src="https://img.shields.io/badge/Qt-UI-lightgrey?logo=qt&logoColor=white" />
  <img alt="Python" src="https://img.shields.io/badge/Python-🐍-informational?logo=python&logoColor=white" />
  <img alt="嘉立创/AD" src="https://img.shields.io/badge/嘉立创_AD-嘉立创_AD-orange?logo=kicad&logoColor=white" />
  &nbsp;
  <!-- 状态类 -->
  <img alt="status" src="https://img.shields.io/badge/status-WIP-yellow" />
  <img alt="license" src="https://img.shields.io/badge/license-MIT-lightgrey" />
  <img alt="GitHub Actions" src="https://img.shields.io/badge/CI-GitHub%20Actions-black?logo=githubactions&logoColor=white" />
</p>

---

## Hi there 👋

> 嘿！您好！这里是Awesome-Embedded!是我们所有嵌入式小产品和采用的底层基础框架的开源仓库集群！
> 集群下的所有仓库中将会包含所有学习工作室的嵌入式小玩具的所有开源资料，包含但是不限于代码，硬件PCB设计和相关的教学文档。

---

## 📦 项目展示

| 仓库 | 描述 | Stars |
|:------|:-----|:-----:|
| [Tutorial_AwesomeModernCPP](https://github.com/Awesome-Embedded-Learning-Studio/Tutorial_AwesomeModernCPP) | 现代 C++ 嵌入式完整教程（C++11–C++23） | ![stars](https://img.shields.io/github/stars/Awesome-Embedded-Learning-Studio/Tutorial_AwesomeModernCPP?style=social) |
| [imx-forge](https://github.com/Awesome-Embedded-Learning-Studio/imx-forge) | i.MX6ULL 芯片 uboot / kernel / rootfs 补丁模板 | ![stars](https://img.shields.io/github/stars/Awesome-Embedded-Learning-Studio/imx-forge?style=social) |
| [Awesome-Embedded](https://github.com/Awesome-Embedded-Learning-Studio/Awesome-Embedded) | 主导航仓库 — 资源聚合入口 | ![stars](https://img.shields.io/github/stars/Awesome-Embedded-Learning-Studio/Awesome-Embedded?style=social) |
| [Project_MakeAMemroyPool](https://github.com/Awesome-Embedded-Learning-Studio/Project_MakeAMemroyPool) | 手搓简易内存池，配套 B 站视频教程 | ![stars](https://img.shields.io/github/stars/Awesome-Embedded-Learning-Studio/Project_MakeAMemroyPool?style=social) |
| [Tutorial_cpp_SimpleIniParser](https://github.com/Awesome-Embedded-Learning-Studio/Tutorial_cpp_SimpleIniParser) | C++ 工程化起步 — 手搓最简单的 INI 解析器 | ![stars](https://img.shields.io/github/stars/Awesome-Embedded-Learning-Studio/Tutorial_cpp_SimpleIniParser?style=social) |

> 更多仓库 → [核心仓库分发地](https://github.com/Awesome-Embedded-Learning-Studio/Awesome-Embedded)

---

## 有任何疑问？

- 如果您对下面某一个具体项目存在疑问，欢迎前往对应项目的Issue下，提出任何您想问的问题，多简单都可以！
- 或者到👉 [Issues](https://github.com/Awesome-Embedded-Learning-Studio/Awesome-Embedded/issues) 下进行提问！

## 有想要找我们聊的？

- 讨论交流请到 [Discussions](https://github.com/orgs/Awesome-Embedded-Learning-Studio/discussions)，现在已经开放，您可以聊任何相关的，问任何感兴趣的内容！

---

## 🗺️ Roadmap

- ✅ **Tutorial_AwesomeModernCPP** — 现代 C++ 教程（主体完成）
- 🚧 **STM32 C++ 实战指南** — 基于现代 C++ 的 STM32 开发实战
- 🚧 **上位机现代 C++ 特性体验** — 桌面端 C++ 新特性学习项目
- 📌 **Docker / Renode 模拟器支持** — 无硬件也能学嵌入式

---

## 目前为止，这个组织都有谁？

<table align="center">
  <tr>
    <td align="center" width="50%">
      <img src="https://github.com/HNHKHNH.png?size=80" /><br />
      <a href="https://github.com/HNHKHNH"><strong>HNHKHNH</strong></a><br />
      硬件大牛，PCB工程师
    </td>
    <td align="center" width="50%">
      <img src="https://github.com/Charliechen114514.png?size=80" /><br />
      <a href="https://github.com/Charliechen114514"><strong>Charliechen114514</strong></a><br />
      一般路过的普通C/C++程序员
    </td>
  </tr>
</table>

> 想加入？欢迎发 Issue 或 PR，也可以直接在下面的联系方式里 @ 我们。

---

## 🤝 参与贡献

想参与？很简单：

1. **Fork** 目标仓库到自己的账号
2. **新建分支** 并进行修改
3. 提交 **Pull Request** 并描述改动内容
4. 等待 **Review**，合并后即成为贡献者

---

## 许可 & 联系方式

- 许可：[![License: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](https://opensource.org/licenses/MIT)
- 邮箱：[725610365@qq.com](mailto:725610365@qq.com)
- GitHub Discussions：[点击进入](https://github.com/Awesome-Embedded-Learning-Studio/Awesome-Embedded/discussions)
- 联系：在 GitHub Issue / PR 中 @ 对应用户名，或者直接用仓库 Discussions。

---

<div align="center">

> 感谢阅读！如果你看到这里了，说明你对嵌入式也有一丁点好奇 —— 欢迎加入我们，一起把小玩具做成好玩的教程 & 项目！🚀

**Keep it small, make it fun — Awesome-Embedded-Learning-Studio** ✨

<img src="https://visitor-badge.laobi.icu/badge?page_id=Awesome-Embedded-Learning-Studio.Awesome-Embedded-Learning-Studio" alt="visitor badge" />

</div>
